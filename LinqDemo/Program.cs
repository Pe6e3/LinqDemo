namespace LinqDemo
{
    public class Program
    {
        ...
        public static int[] S2 = new int[] { 57, -6, 88, -64, 79, 94, -66, 28, };
        public static string[] A = { "1hat", "2door", "5computer", "sunglasses", "OBJ", "5ocean", "watch", "piano", "guitar", "5shoe", "spoon", "river", "beach", "phone", "camera", "key", "mountain", "book", "cloud", "bottle", "tree", "flower", "car", "house", "moon", "balloon", "zebra", "pen", "dog", "chair", "cup", "sun" };
        ...
        public static void Solutions()
        {
            ...
            task[1] = "1. Дана целочисленная последовательность, содержащая как положительные, так и отрицательные числа. " +
                      "Вывести ее первый положительный элемент и последний отрицательный элемент.";
            result[1] = "First plus = " + S.FirstOrDefault(p => p > 0) + " Last Minus = " + S.LastOrDefault(p => p < 0);
            ...
            result[2] = S.FirstOrDefault(p => p > 0 & p % 10 == D).ToString();
            ...
            try
            {
                if (A.Count(p => p.EndsWith(C.ToString())) == 1)
                    result[4] = A.SingleOrDefault(p => p.EndsWith(C)).ToString();
                if (A.Count(p => p.EndsWith(C.ToString())) == 0)
                    result[4] = "";
                else result[4] = A.SingleOrDefault(p => p.EndsWith(C.ToString()));
            }
            catch
            {
                result[4] = "Error";
            ...
            task[6] = "6. Дана строковая последовательность A \n" +
                              "Найти сумму длин всех строк, входящих в данную последовательность";
            result[6] = A.Sum(p => p.Length).ToString();


            task[7] = "7. Дана целочисленная последовательность S \n" +
            ...
            task[8] = "8. Дана целочисленная последовательность S \n" +
            ...
            int res1 = S.Count(s => s > 0 && s.ToString().Length == 2);
            double res2 = S.Where(s => s > 9 & s < 100).Average(s => s);
            result[8] = "Количество положительных двузначных: " + res1.ToString() + ", их среднее арифметическое: " + res2.ToString();
            ...
            result[10] = A.Where(p => p.Length == L).Max();


            task[11] = "11. Дана последовательность непустых строк A \n" +
            ...
            task[12] = "12. Дана целочисленная последовательность S. Найдите произведение последних цифр всех элементов последовательности, используя метод Aggregate. Чтобы избежать целочисленного переполнения, при вычислении произведения используйте вещественный числовой тип.";
            double test = S2.Aggregate(1.0, (acc, x) => acc * (x % 10));
            result[12] = test.ToString();

            task[13] = "13. Дано целое число D (> 0). Используя методы Range и Sum, найдите сумму ряда 1 + (1/2) + … + (1/D) в виде вещественного числа.";
            result[13] = Enumerable.Range(1, D)
                .Select(p => 1.0 / p)
                .Sum()
                .ToString();
            ...
            result[14] = S.ToList().Where(p => p > 0).Aggregate("", (x, y) => x + " " + y) + " второй способ:\n  ";
            ...
            foreach (var item in S.Where(p => p < 0 && p % 2 == 0).Reverse())
                result[16] += item + " ";
